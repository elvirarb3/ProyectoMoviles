# solo vale en los PCs del laboratorio Y con los pendrives de ASUS. Versión 2223
if [ $# -ne 1 ]
  then
    ...
    #alias para abreviar el comando de conexion con el robot
    if [ $1 -eq 5 ]
      then
        login=tb2
      else
    ...
    echo alias sshrobot=\"sshpass -p ros ssh $login@$ip_turtlebot\" >> ~/.bashrc
fi
