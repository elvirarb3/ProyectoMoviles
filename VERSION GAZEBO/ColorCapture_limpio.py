...
class DetectColors():
    def __init__(self):
        ...
        #DETECTAMOS EL AZUL
        self.hue_blue_l = 95 #90 depende luz
        self.hue_blue_h = 130 #140
        self.saturation_blue_l = 50 #100
        self.saturation_blue_h = 255
        self.lightness_blue_l = 0 #131
        self.lightness_blue_h = 255
    ...
    def FindColor(self):
        cv_image_mask = self.ColorVerde()
        
        cv_image_mask = self.ColorAzul()
        
        cv_image_mask = self.ColorRojo()
    ...
    def ColorRojo(self):
        ...
        col = cv2.cvtColor(res, cv2.COLOR_HSV2BGR) # pasar de HSV a color
        gray = cv2.cvtColor(col, cv2.COLOR_BGR2GRAY) # pasar de color a gris
        im, contours, hi = cv2.findContours(gray, 1,2) # obtener contornos
